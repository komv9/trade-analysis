...
<body>
	...
	<div class="main">
	<h2 style="text-align: center; margin-top: 5%; padding: 5px;"><?php echo $monthname;?></h2>
	<?php 
	    $m = "SELECT * FROM `trade` WHERE MONTH(dates)=$curr_month ORDER BY dates DESC;";
	    $monthwise = mysqli_query($mysqli, $m);
            if(!$monthwise) {
    	        echo "Failed";
            }
            if ($monthwise->num_rows>0) {
	        while($row = $monthwise->fetch_assoc()) {
	    ?>
	    <article id="post">
	    	...
	    	<div class="wrap">
        <div class="control" onclick="location.href='<?php echo "month.php?todelete=".$row['sno'];?>';">Delete</div>
        ...
        if(isset($_GET['todelete'])) {
            ...
            if ($pr) {
            //echo $todelete;
            echo "<script>window.location = 'month.php'</script>";
            }
          }
        ?>
	    <section>
